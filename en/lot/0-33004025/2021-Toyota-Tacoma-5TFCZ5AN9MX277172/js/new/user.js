    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            if (response.logged === 1) {
                ...
                if(response.support_mail.available == 1) {
                    $("#header-email-link").attr("href", "mailto:" + response.support_mail.mail).html('<img src="https://bid.cars/images/contact/email.svg" alt="Email" /> Email');
                    $("#contact-email-link").attr("href", "mailto:" + response.support_mail.mail).text(response.support_mail.mail);
                }
